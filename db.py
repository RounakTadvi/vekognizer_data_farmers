...
class DBInterface:
    ...
    def __init__(self, config: Config):
        ...
        self.client: AstraClient = create_client(
        ...
        self.tables_path = (
            f"/api/rest/v2/schemas/keyspaces/{config.ASTRA_DB_KEYSPACE}/tables"
        )
        self.vehicles_by_color_type_path = (
            f"/api/rest/v2/keyspaces/{config.ASTRA_DB_KEYSPACE}/{Const.TABLE_VEHICLES_BY_COLOR_AND_TYPE}"
        )
        self.vehicles_by_color_path = (
            f"/api/rest/v2/keyspaces/{config.ASTRA_DB_KEYSPACE}/{Const.TABLE_VEHICLES_BY_COLOR}"
        )
    ...
    def __get_vehicles_by_color_type_schema(self) -> dict:
        ...
        """
        return {
            "name": Const.TABLE_VEHICLES_BY_COLOR_AND_TYPE,
            "columnDefinitions": [
                {"name": Const.COL_COLOR, "typeDefinition": "text"},
                {"name": Const.COL_TYPE, "typeDefinition": "text"},
                {"name": Const.COL_LATITUDE, "typeDefinition": "decimal"},
                {"name": Const.COL_LONGITUDE, "typeDefinition": "decimal"},
                {"name": Const.COL_DATE, "typeDefinition": "date"},
                {"name": Const.COL_TIME, "typeDefinition": "time"},
                {"name": Const.COL_IMAGE_URL, "typeDefinition": "text"},
                {"name": Const.COL_FEATURE_VECTOR_PATH, "typeDefinition": "text"},
                {
    ...
    def __get_vehicles_by_color_schema(self) -> dict:
        ...
        """
        return {
            "name": Const.TABLE_VEHICLES_BY_COLOR,
            "columnDefinitions": [
                {"name": Const.COL_COLOR, "typeDefinition": "text"},
                {"name": Const.COL_LATITUDE, "typeDefinition": "decimal"},
                {"name": Const.COL_LONGITUDE, "typeDefinition": "decimal"},
